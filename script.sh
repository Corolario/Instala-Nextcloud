...
# Links para download
GUACA_SERVER=http://apache.org/dyn/closer.cgi?action=download&filename=guacamole/0.9.14/source/guacamole-server-0.9.14.tar.gz
GUACA_CLIENT=http://apache.org/dyn/closer.cgi?action=download&filename=guacamole/0.9.14/binary/guacamole-0.9.14.war

# Senha do root MySQL
mysqlrootpassword=
# Senha do usuario db Guacamole
guacdbuserpassword=
...
wget -O guacamole-server.tar.gz $GUACA_SERVER
if [ $? -ne 0 ]; then
    echo "O download do Guacamole Server falhou."
    exit
fi

# Download Guacamole Client
wget -O guacamole.war $GUACA_CLINT
if [ $? -ne 0 ]; then
    echo "O download do Guacamole Clint falhou."
    exit
fi
